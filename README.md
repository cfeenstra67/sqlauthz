...
| `reconcile`<br/>`--reconcile`<br/>`SQLAUTHZ_RECONCILE` | No | `false` | Reconcile direct privileges and restrictive RLS policies instead of replacing them unconditionally. Only changed privileges and policies emit statements, which can substantially reduce permission audit log volume. Role memberships are still revoked from managed users. |
...
| `dryRunShort`<br/>`--dry-run-short`<br/>`SQLAUTHZ_DRY_RUN_SHORT` | No | `false` | Print the permission statements without setup, teardown, or transaction statements, instead of executing anything. This includes RLS statements and, in reconciliation mode, both sides of the privilege delta. Note that if setting this via environment variable, the value must be `true`. This conflicts with `dryRun` |
...
Passing `--reconcile` changes how direct schema, table, column, view, function, procedure, and sequence privileges and restrictive RLS policies are applied. `sqlauthz` reads the current state and emits only the difference from the desired configuration. Reapplying an unchanged configuration therefore emits no permission-changing statements.
...
Existing role memberships are still revoked from managed users. Restrictive RLS policies are compared by command, roles, `USING`, and `WITH CHECK`; changed policies are dropped and recreated, while unchanged policies emit no statements. Catalog expressions that cannot be safely normalized are treated as changed.
