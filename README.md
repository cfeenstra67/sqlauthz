...
| `reconcile`<br/>`--reconcile`<br/>`SQLAUTHZ_RECONCILE` | No | `false` | Reconcile direct privileges instead of revoking and re-granting all of them. Only changed privileges emit `GRANT` or `REVOKE` statements, which can substantially reduce permission audit log volume. Role memberships are still revoked from managed users, and RLS policies retain their existing behavior. |
...
Passing `--reconcile` changes how direct schema, table, column, view, function, procedure, and sequence privileges are applied. `sqlauthz` reads the direct privileges currently assigned to managed users and emits only the difference from the desired configuration. Reapplying an unchanged configuration without restrictive RLS policies therefore emits no permission-changing statements.
